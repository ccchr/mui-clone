...
(function() {
	...
	function animateOnScroll() {
		...
		jQuery('.has-animation').each(function() {
			...
			if (jQuery(window).width() > 1024) {
				...
				if (thisItem.hasClass("animated")) {} else if (visible) {
		...
		jQuery('.progress-bar-item').each(function() {
			...
			if (thisItem.hasClass("anim")) {} else if (visible) {
				var randomval = Math.floor(Math.random() * (300 - 50 + 1)) + 50;
				thisItem.addClass("anim");
				thisItem.find('.progress-bar .progress-active ').animate({
					'width': percent + '%',
	...
	function reorganizeIsotope() {
		jQuery('.isotope-grid[class*="style-modern"]').each(function() {
			...
			$container.children('.isotope-item').css({
				'height': height + 'px'
			});
			$container.children('.isotope-item.tall, .isotope-item.wide-tall').css({
			...
			$container.children('.isotope-item').each(function() {
				...
				if (imgRatio < itemRatio) {
					imgClass = 'wide-img';
					jQuery(this).find("img").removeClass('tall-img');
				} else {
					imgClass = 'tall-img';
					jQuery(this).find("img").removeClass('wide-img');
				}
	...
	jQuery(document).ready(function() {
		...
		if (jQuery(".dribbble-widget").length > 0) {
			jQuery('.dribbble-widget').each(function() {
				...
				jQuery.jribbble.users(dribbbleUser).shots({
